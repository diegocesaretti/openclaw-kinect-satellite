...
| Wake word | `Alexa` | Select `Alexa` or `Jarvis`; the pinned official openWakeWord ONNX classifier, shared feature models, and license are retrieved automatically on first start. There is no editable model path. |
| Threshold | `0.5` | Number from 0 through 1. |
...
Downloaded openWakeWord v0.5.1 ONNX assets are stored beneath the per-user `models` directory. Each entry contains `metadata.json` with SHA-256 values, `ATTRIBUTION.txt` with pinned official URLs, and the CC BY-NC-SA 4.0 legal code. Cached bytes are checked before every load. Pre-trained models are non-commercial and require attribution and ShareAlike compliance.
